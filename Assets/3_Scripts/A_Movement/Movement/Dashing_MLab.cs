...
{
    // deactivate dashing (use LockDashing() function)
    private bool locked;
    ...
    [Header("CoreImplementation")]
    private const string identifier_dashing = "Dash";

    [Header("Effects")]
    public float dashFov = 95f;
    public AudioClipData dashSound;
    ...
    private void Dash()
    {
        ...
        {
            core.RemoveGravityAffectedUpwardsVelocities();
            core.ResetGravity();
        }

        // add the dash force
        core.AddForce(identifier_dashing, -1, force.magnitude, new Direction(Direction.Type.FixedVector, force.normalized), dashDuration);

        // make sure the dash stops after the dashDuration is over
        Invoke(nameof(ResetDash), dashDuration);

        // audio
        SoundManager.PlayEffect(dashSound);
    }
    ...
    private Vector3 GetDirection(Transform forwardT)
    {
        ...
        float x = input.horizontalInput;
        float z = input.verticalInput;

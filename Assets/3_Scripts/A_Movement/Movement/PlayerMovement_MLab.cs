...
{
    // deactivate all movement (use LockMovement() function)
    private bool locked;
    ...
    // PlayerStats -> baseMoveSpeed = walkMaxSpeed;
    public float sprintMaxSpeedIncrease = 3f;
    ...
    [HideInInspector] public PlayerCam_MLab cam;
    ...
    {
        // implement overallMaxSpeedBoost
        float realMaxSpeed = maxSpeed;
        float realMaxYSpeed = maxYSpeed;
        ...
        if (rbFlatVelocity.magnitude > realMaxSpeed)
        {
            // ...then first calculate what your maximal velocity would be
            Vector3 limitedFlatVelocity = rbFlatVelocity.normalized * realMaxSpeed;
        ...
        if(maxYSpeed != -1 && currYVel > realMaxYSpeed)
        {
            // ...just set your rigidbodys y velocity to you maxYSpeed, while leaving the x and z axis untouched
            rb.velocity = new Vector3(rb.velocity.x, realMaxYSpeed, rb.velocity.z);
        }
    }
    ...
    public void AddForce(Vector3 force, float maxSpeed)
    {
        if (maxSpeed != -1)
            EnableLimitedState(maxSpeed);
        else
            dashing = true;

        rb.AddForce(force, ForceMode.Impulse);

        Invoke(nameof(ResetAddForce), 0.25f);
    }
    private void ResetAddForce()
    {
        DisableLimitedState();
    ...
    private void StateHandler()
    {
        ...
        float sprintMaxSpeed = stats.moveSpeed + sprintMaxSpeedIncrease;
        ...
        else if (grounded && walking)
        {
            ...
            desiredMaxSpeed = stats.moveSpeed;
        }
        ...
        else
        {
            ...
            if (desiredMaxSpeed < stats.moveSpeed)
                desiredMaxSpeed = sprintMaxSpeed;
    ...
    public void LockPlayerMovement(bool lockAllMovementScripts)
    {
        if (lockAllMovementScripts)
        {
            PlayerReferences.instance.dashing.Lock();
        }
    ...
    {
        PlayerReferences.instance.dashing.Unlock();
